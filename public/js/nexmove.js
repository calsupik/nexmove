...
var urlString = 'https://nexmove.herokuapp.com'; //Locations Array

var locations = []; //Map Variable

var map = null; //Current Location

var currentLocation = null; //Current Location Radius

var currentLocationRadius = null; //Nearby Locations Radius

var locationsDistance = 100000; //Navigation Watch ID	

var watchID = null; //Application Function	
...
var app = {
  ...
  recievedEvent: function recievedEvent(id) {
    if (id == 'deviceready') {
      app.deviceReady();
    }
  },
  ...
  initMap: function initMap() {
    ...
    if (window.backgroundGeolocation) {
      backgroundGeolocation.start(); //Turn Off Background Geolocation
      //backgroundGeolocation.stop(); 
    } //Get Current Location	


    navigator.geolocation.getCurrentPosition(app.onInitSuccess, app.onError); //Navigation Watch Options
  ...
  onInitSuccess: function onInitSuccess(position) {
    //Sets Current LatLng Variable
    var latlng = new google.maps.LatLng(position.coords.latitude, position.coords.longitude); //Set Map Center

    map.setCenter(position.coords.latitude, position.coords.longitude); //Set currentLocation Position

    currentLocation.setCenter(latlng); //Set currentLocationRadius Position

    currentLocationRadius.setCenter(latlng); //Get Nearby Locations from Database based off Current Location

    app.getLocations(null); //onSuccess Call
  ...
  onSuccess: function onSuccess(position) {
    //Sets Current LatLng Variable
    var latlng = new google.maps.LatLng(position.coords.latitude, position.coords.longitude); //Set currentLocation Position

    currentLocation.setCenter(latlng); //Set currentLocationRadius Position

    currentLocationRadius.setCenter(latlng); //Loop through Geofences
    ...
    locations.forEach(function (location) {
      ...
      if (map.checkGeofence(position.coords.latitude, position.coords.longitude, location.geofence)) {
        ...
        if (location.inside) {//Location is already inside geofence
        } else {
          //Sets location inside geofence
          location.setInside(); //Creates Notification Object
  ...
  getLocations: function getLocations(type) {
  ...
  loadLocations: function loadLocations(json) {
    locations = [];
    var list = '<div class="container"><div class="row">';
    var details = ''; //for (var i = 0; i < json.length; i++) {

    json.forEach(function (location) {
      //Creates Location in List
      list += '<div class="col-md-4 col-sm-6 locations-item text-center">' + '<a id="location' + location.id + '" href="#locationDetail' + location.id + '" class="locations-link" data-toggle="modal">' + '<div class="locations-hover">' + '<div class="locations-hover-content">' + '</div>' + '</div>' + '<img src="' + urlString + '/images/' + location.img + '"  class="img-responsive img-rounded" alt="">' + '</a>' + '<div class="locations-caption">' + '<h4>' + location.name + '</h4>' + '<p class="text-muted">' + location.short_desc + '</p>' + '</div>' + '</div>'; //Creates Location Details Popup Modal

      details += '<div class="locations-modal modal fade text-center" id="locationDetail' + location.id + '" tabindex="-1" role="dialog" aria-hidden="true" style="visibility:visible" >' + '<div class="vertical-alignment-helper">' + '<div class="modal-dialog vertical-align-center">' + '<div class="modal-content">' + '<div class="close-modal" data-dismiss="modal">' + '<div class="lr">' + '<div class="rl">' + '</div>' + '</div>' + '</div>' + '<div class="container">' + '<div class="row">' + '<div class="col-lg-8 col-lg-offset-2">' + '<div class="modal-body">' + '<h2>' + location.name + '</h2>' + '<p class="item-intro text-muted">' + location.short_desc + '</p>' + '<img class="img-responsive" src="' + urlString + '/images/' + location.img + '" alt="">' + '</br>' + '<p>' + location.long_desc + '</p>' + '<button type="button" class="btn btn-danger" data-dismiss="modal"><i class="fa fa-times"></i> Close Details</button>' + '</div>' + '</div>' + '</div>' + '</div>' + '</div>' + '</div>' + '</div>' + '</div>'; //Create New Location Object

      var locationObj = new app.Location(location); //Push Location Object onto Array of Locations
  ...
  Location: function Location(location) {
    ...
    */
    ...
    this.setInside = function () {
      this.inside = true; //this.marker.setClickable(true);
      //this.marker.setOpacity(1.0);
    };

    this.setOutside = function () {
      this.inside = false; //this.marker.setClickable(true);
      //this.marker.setOpacity(1.0);
    };
  },
  //Filter Locations
  filter: function filter(type) {
    ...
    map.refresh(); //Get Current Location	
